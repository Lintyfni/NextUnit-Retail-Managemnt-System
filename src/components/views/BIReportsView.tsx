...
export const BIReportsView: React.FC = () => {
  const { orders, products, currency, language } = useApp();
  ...
  const handleExportCSV = () => {
    ...
    link.setAttribute("download", `NextUnit_Sales_Report_${new Date().toISOString().slice(0, 10)}.csv`);
  ...
  return (
    <div id="bi-reports-view" className="space-y-5 animate-fade-in text-slate-800">
      {/* Top Header */}
            ...
            {topSellers.map((item, idx) => (
              ...
              >
                <div>
                  ...
                  <div className="text-[10px] text-slate-500">{item.category} • {item.unitsSold} units sold</div>
                </div>
                <div className="text-right">
                  <div className="font-mono font-bold text-emerald-700 text-xs">
                    {formatCurrency(item.revenue, currency, language)}
                  </div>
                  <span className="text-[10px] text-emerald-800 font-semibold">Rank #{idx + 1}</span>
                </div>
              </div>
            ))}
          </div>
          ...
          <div className="space-y-3 text-xs">
            <div className="space-y-1">
              <div className="flex justify-between">
                <span className="text-slate-700 font-medium">Smartphones & Flagship Devices</span>
                <span className="font-mono font-bold text-slate-900">54% (32.4M MMK)</span>
              </div>
            ...
            <div className="space-y-1">
              <div className="flex justify-between">
                <span className="text-slate-700 font-medium">Laptops & Computing</span>
                <span className="font-mono font-bold text-slate-900">28% (16.8M MMK)</span>
              </div>
              <div className="w-full bg-slate-200 h-2 rounded-full overflow-hidden">
            ...
            <div className="space-y-1">
              <div className="flex justify-between">
                <span className="text-slate-700 font-medium">Audio & Accessories (High Margin 45%)</span>
                <span className="font-mono font-bold text-slate-900">18% (10.8M MMK)</span>
              </div>
              <div className="w-full bg-slate-200 h-2 rounded-full overflow-hidden">
